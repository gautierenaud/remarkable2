...
class rMCloudPlugin(InterfaceAction):
    ...
    action_spec = ('Upload to reMarkable', None,
                   'Upload selected books to reMarkable\'s cloud', None)
    ...
    def _get_selected_ids(self):
        ...
        if not rows or len(rows) == 0:
            d = error_dialog(
                self.gui, 'Upload books to reMarkable Cloud', _('No book selected'))
            d.exec_()
            return set()
        return set(map(self.gui.library_view.model().id, rows))

    def upload_books(self):
        from calibre_plugins.remarkable_cloud.jobs import (
            get_upload_books_job, update_token)
        update_token()
        ...
        self.gui.job_manager.run_threaded_job(
            get_upload_books_job(compatible_books, db))
