...
from calibre_plugins.remarkable_cloud_driver.config import dump, config
...
class Client(object):
    ...
    def register_device(self, code: str):
        ...
        response = self.request("POST", DEVICE_TOKEN_URL, data=json.dumps(body),
                                headers={"Content-type": "application/json"})
        if response.ok:
            config["devicetoken"] = response.text
            return True
        else:
    ...
    def renew_token(self):
        ...
        if response.ok:
            config["usertoken"] = response.text
            dump()
            return True
        else:
    ...
    def is_auth(self) -> bool:
        ...
        if config["devicetoken"] and config["usertoken"]:
            return True
        else:
            return False

    def is_registered(self) -> bool:
        """Is the client is registered to remarkable's cloud

        Returns:
            bool: True if the client is registered
        """
